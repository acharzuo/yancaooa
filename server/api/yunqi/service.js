...
exports.getGandefu = function(date){
    var fivesix = new FiveSix(date);
    return fivesix.getGandefu();
}

// 获取五运六气信息
exports.getWuyunliqi = function(date) {
    var fivesix = new FiveSix(date);
    var sitianZaiquan = fivesix.getSitianZaiquan();
    var zhongyun = fivesix.getZhongyun();
    var result = {
        date: date,
        nongli: fivesix.getNongli(),
        ganzhi: fivesix.getGanZhi(),
        jieqi: fivesix.getJieqi(),
        sitian: sitianZaiquan[0],
        sitianZaiquan: sitianZaiquan,
        zaiquan: sitianZaiquan[1],
        suiyun: (zhongyun[0] ? '太' : '少') + zhongyun[1],
        zhongyun: zhongyun,
        zhuKeYun: fivesix.getZhuKeYun(),
        zhuKeQi: fivesix.getZhuKeQi(),
        jiaoyun: fivesix.getJiaoyun(),
        jiaoqi: fivesix.getJiaoqi(),
        gandefu: fivesix.getGandefu(),
        currentYunqi: fivesix.getCurrentYunQiIndex()
    }
    console.log(result)
    return result;
};
