...
function xingHY(xt,t){ //行星合月(视赤经),t儒略世纪TD
 ...
 E = hcjj(t);
 ZB.nutation(t);
 g=new Array(d[2], d[3], ZB.dL, ZB.dE); //光行时,章动
...
function xingX(xt,jd,L,fa){ //行星计算,jd力学时
 ...
 if(xt==10){ //月亮
  ...
  //地心黄道及地心赤道
  z[0] = rad2mrad(z[0]+ZB.dL); 
  s += '视黄经 ' +rad2str(z[0],0) +' 视黄纬 ' +rad2str(z[1],0) +' 地心距 ' +toFixed(ra,rfn)+'\r\n';
  z = llrConv(z,E); //转到赤道坐标
  s += '视赤经 ' +rad2str(z[0],1) +' 视赤纬 ' +rad2str(z[1],0) +' 光行距 ' +toFixed(rb,rfn)+'\r\n';
 ...
 if(xt<10){ //行星和太阳
  ...
  z[0] = rad2mrad(z[0]);
  s += '黄经一 ' +rad2str(z[0],0) +' 黄纬一 ' +rad2str(z[1],0) +' 向径一 ' +toFixed(z[2],rfn)+'\r\n';
 ...
 s += '站赤经 ' +rad2str(z[0],1) +' 站赤纬 ' +rad2str(z[1],0) +' 视距离 ' +toFixed(rc,rfn)+'\r\n';
...
function lineEar2(x1,y1,z1, x2,y2,z2, e,r, I){ //I是贝塞尔坐标参数
  ...
  p.J=p.W=100;
  if(p.D<0) return p;
  p.J = rad2rrad(I[2]-atan2(p.y,p.x)-I[0]);
...
function lineEar(P,Q,gst){ //在分点坐标中求空间两点连线与地球的交点(靠近点P的交点),返回地标
  ...
  r.J = rad2rrad( gst-atan2(r.y,r.x) );
  return r;
}

function ovlOvl(R,x0,y0,R2){ //两圆的交点,R主圆半径,R2次圆半径,x0,y0次圆圆心
  var re=new Object();
  var d=sqrt(x0*x0+y0*y0);
  var sinB=y0/d, cosB=x0/d;
  var cosA=(R*R+d*d-R2*R2)/2/d/R;
  if(abs(cosA)>1){ re.n=0; return re; } //无解
  if(abs(cosA)==1) re.n=1; else re.n=2; //单或双解
...
function lineOvl(x1,y1,dx,dy,r){
  var A,B,C,D,R,t1,t2, p=new Object();
  A = dx*dx + dy*dy;
  B = x1*dx + y1*dy;
  C = x1*x1 + y1*y1 - r*r;
  ...
  R = sqrt(dx*dx + dy*dy);
  p.R1 = R*abs(t1); //x1到交点1的距离
  p.R2 = R*abs(t2); //x1到交点2的距离
  return p;
}
...
function sun_moon(){
 ...
 this.toHTML=function(fs){
  ...
  s += '时差 ' + m2fm(this.sc*86400,2,1) + " 月亮被照亮 " + toFixed(this.mIll*100,2)+'% ';
  s += '</td></tr>';
  ...
  s += '距离     ' + toFixed(this.mR,2)  +'千米     '+ toFixed(this.sR,8)+'AU'+'\r\n';
  s += '</pre></center></td></tr>';
  ...
  if(fs){
   ...
   s += ' ΔT=' + toFixed(this.dt*86400,1) +'秒<br>';
   s += '黄经章 '+toFixed(this.dL/pi2*360*3600,2) +'" ';
   s += '交角章 '+toFixed(this.dE/pi2*360*3600,2) +'" ';
...
//====================================
function rsGS(dt,n){ //日食星历根数
 this.Zs=new Array();  //日月赤道坐标插值表
 this.sun0 = new Array();
 this.Zjd=0, this.Zdt=dt, this.Zn=n,this.jing=0;
 this.eR0 = 0.99834; //地球平均半径
 this.init=function(jd,jing) { //准备根数表
  var kn1=Math.floor((jd+8)/29.5306), kn2=Math.floor((this.Zjd+8)/29.5306);
  if(this.Zjd && kn1==kn2 && this.jing==jing) return;
  this.jing = jing;
  this.Zs.length = 0;

  this.Zjd = jd = XL.MS_aLon_t2( Math.floor((jd+8)/29.5306)*Math.PI*2 )*36525; //低精度的朔(误差10分钟),与食甚的误差1到2小时
  this.E = hcjj(jd/36525); //黄赤交角
  this.dT = dt_T(jd); //deltat T

  var i,k,E,T, S,M,B, a=this.Zs;

  for(i=0;i<this.Zn;i++){ //插值点范围不要超过360度(约1个月)
   T=( this.Zjd + (i-this.Zn/2+0.5)*this.Zdt ) / 36525;
   ZB.nutation(T);
   E = this.E+ZB.dE;

   if(jing==0) S = e_coord(T,-1,-1,-1), M = m_coord(T,-1,-1,-1);   //地球坐标及月球坐标,全精度
   if(jing==1) S = e_coord(T,65,65,65), M = m_coord(T,-1,150,150); //中精度
   if(jing==2) S = e_coord(T,20,20,20), M = m_coord(T,30,30,30);   //低精度

   S[0] = S[0]+ZB.dL+gxc_sunLon(T)+Math.PI;  S[1] = -S[1] + gxc_sunLat(T);  //补上太阳光行差及章动
   M[0] = M[0]+ZB.dL+gxc_moonLon(T);         M[1] =  M[1] + gxc_moonLat(T); //补上月球光行差及章动
   ...
   a[k+6]=B[0], a[k+7]=B[1], a[k+8]=pGST(T*36525-this.dT, this.dT)+ZB.dL*cos(E); //真恒星时
  }
  var p = a.length - a.length/this.Zn;
  for(i=0;i<3;i++) this.sun0[i] = (a[i]+a[p+i])/2; //日食期间太阳平均位置
 };
 this.chazhi=function(jd,p,m){//日月坐标快速计算(贝赛尔插值法),计算第p个根数开始的m个根数
  var i, N=this.Zn, B=this.Zs, z=new Array(), n0;
  var w = B.length/this.Zn; //每节点个数
 ...
 this.sun =function(jd){ return this.chazhi(jd,0,3); }; //传回值可能超过360度
 this.moon=function(jd){ return this.chazhi(jd,3,3); };
 this.bse =function(jd){ return this.chazhi(jd,6,3); };

 this.cd2bse=function(jd,z){ //赤道转贝塞尔坐标
  var b=this.bse(jd);
  var r=new Array(z[0]-b[0],z[1],z[2]);
  r = llrConv(r,-b[1]);
  return llr2xyz(r);
 };
 this.bse2cd=function(jd,z){ //贝塞尔转赤道坐标
  var b=this.bse(jd);
  var r = xyz2llr(z);
  r = llrConv(r,b[1]);
  r[0] = rad2mrad(r[0]+b[0]);
  return r;
 };

 this.bseM=function(jd){ var a=this.cd2bse(jd,this.chazhi(jd,3,3)); a[0]/=cs_rEar, a[1]/=cs_rEar;  return a; }; //月亮的贝塞尔坐标
 this.bseS=function(jd){ var a=this.cd2bse(jd,this.chazhi(jd,0,3)); a[0]/=cs_rEar, a[1]/=cs_rEar;  return a; }; //太阳的贝塞尔坐标


 //以下计算日食总体情况
 this.fastTest=function(jd){ //快速估计有无日食,排除不可能的朔,jd为平朔或定朔
  var T=jd/36525, T2=T*T,T3=T2*T,T4=T3*T;
  var F =93.2720993+483202.0175273*T-0.0034029*T2-T3/3526000+T4/863310000;
  F = abs(sin(F/180*Math.PI));
  if(F>0.5) return 0; //其实大于21度已不可能
  return 1;
 };
 this.bse2db=function(z,I,f){ //贝赛尔转地标(p点到原点连线与地球的交点,z为p点直角坐标),f=1时把地球看成椭球
  var r = xyz2llr(z);
  r = llrConv(r,I[1]);
  r[0] = rad2rrad(I[2]-(r[0]+I[0]));
  if(f) r[1] = atan( tan(r[1])/cs_ba2 );
  return r;
 };
 this.bseXY2db=function(x,y,I,f){ //贝赛尔转地标(过p点垂直于基面的线与地球的交点,p坐标为(x,y,任意z)),f=1时把地球看成椭球
  var b=f?cs_ba:1;
  var F = lineEar2(x,y,2,  x,y,0,  b,1,I);//求中心对应的地标
  return [F.J,F.W];
 },

 this.Vxy=function(x,y,s, vx,vy){ //地球上一点的速度，用贝塞尔坐标表达，s为贝赤交角
   var r = new Object();
   var h = 1-x*x-y*y; if(h<0) h=0;
   r.vx = pi2*( sin(s)*sqrt(h)-cos(s)*y );
   r.vy = pi2*x*cos(s);
   r.Vx = vx - r.vx;
   r.Vy = vy - r.vy;
   r.V = sqrt(r.Vx*r.Vx+r.Vy*r.Vy);
   r.cosA = r.Vx/r.V, r.sinA = r.Vy/r.V;
   return r;
 };
 this.rSM=function(mR,sR){ //rm,rs单位千米
  var re=new Object();
  re.mr = cs_sMoon_nasa/mR; //月亮视半径
  re.sr = 959.63/sR*cs_AU;  //太阳视半径
  re.r1 = (re.mr-re.sr)/(1/mR-1/sR)/rad/cs_rEar; //本影半径,地球赤道半径为1
  re.r2 = (re.mr+re.sr)/(1/mR-1/sR)/rad/cs_rEar; //半影半径,地球赤道半径为1
  re.sf = re.mr/re.sr; //食分
  re.ar1 = abs(re.r1);
  return re;
 };
 this.feature=function(jd,jing){//日食的基本特征,jing=1为中精度,jing=2为底度
  this.init(jd,jing);
  jd = this.Zjd; //低精度的朔(误差10分钟)

  var tg=0.04, re=new Object(), ls;
  var a=this.bseM(jd-tg/2);
  var b=this.bseM(jd+tg/2);
  ...
  var xy=dx*dx+dy*dy, xyf=sqrt(xy), l=-(a[0]*dx+a[1]*dy)/xy;

  //中点特征参数
  re.jd = l*tg+jd-tg/2;   //中点时间
  re.xc = l*dx+a[0];      //中点坐标x
  re.yc = l*dy+a[1];      //中点坐标y
  re.zc = l*dz+a[2] + 53*xy*(1-l)*l; //中点坐标z,加了圆补偿,被偿后误差小于0.5千米
  re.vx = dx/tg;          //影速x
  re.vy = dy/tg;          //影速y
  re.Rs = this.sun0[2];   //太阳平均向径
  re.I  = this.bse(re.jd);//中心点的贝塞尔z轴的赤道坐标及恒星时，(J,W,g)

  //影足线特征参数
  re.k  = dy/dx; //斜率
  re.D  = (a[1]*b[0]-a[0]*b[1])/xyf; //直线到圆心的距离,有正负
  re.d  = abs(re.D);     //直线到圆心的距离
  re.ds = -atan(tan(this.E)*sin(re.I[0])); //黄交线与赤交线的夹角

  //影轴交点判断
  var F = lineEar2(re.xc,re.yc,2,  re.xc,re.yc,0,  cs_ba,1,re.I);//求中心对应的地标
  //四个关键点的影子半径计算
  var Bc,Bp,B2,B3,  dl,l2,l3,l4,l5,l6;
  Bc=Bp=B2=B3 = this.rSM(re.zc,re.Rs); //中点处的影子半径
  if(F.W!=100)  Bp = this.rSM(re.zc - F.R2*cs_rEar, re.Rs - F.R2*cs_rEar);
  if(re.d<1){
    dl=sqrt(1-re.d*re.d)/xyf; l2=l-dl, l3=l+dl; //中心始终参数
    B2 = this.rSM(l2*dz+a[2]+53*xy*(1-l2)*l2, re.Rs); //中心线始影半径
    B3 = this.rSM(l3*dz+a[2]+53*xy*(1-l3)*l3, re.Rs); //中心线终影半径
  }
  ls = 1;        dl=0; if(re.d<ls) dl=sqrt(ls*ls-re.d*re.d)/xyf; l2=l-dl, l3=l+dl; //偏食始终参数,l2,l3
  ls = 1+Bc.r2;  dl=0; if(re.d<ls) dl=sqrt(ls*ls-re.d*re.d)/xyf; l4=l-dl, l5=l+dl; //偏食始终参数,l4,l5
  l6 = -a[0]/dx; //视午参数l6
  ls=l2*tg+jd-tg/2; re.gk1=this.bse2db( [l2*dx+a[0],l2*dy+a[1],0], this.bse(ls), 1);  re.gk1[2]=ls; //中心始
  ls=l3*tg+jd-tg/2; re.gk2=this.bse2db( [l3*dx+a[0],l3*dy+a[1],0], this.bse(ls), 1);  re.gk2[2]=ls; //中心终
  ls=l4*tg+jd-tg/2; re.gk3=this.bse2db( [l4*dx+a[0],l4*dy+a[1],0], this.bse(ls), 1);  re.gk3[2]=ls; //偏食始
  ls=l5*tg+jd-tg/2; re.gk4=this.bse2db( [l5*dx+a[0],l5*dy+a[1],0], this.bse(ls), 1);  re.gk4[2]=ls; //偏食终
  ls=l6*tg+jd-tg/2; re.gk5=this.bseXY2db(l6*dx+a[0],l6*dy+a[1],    this.bse(ls), 1);  re.gk5[2]=ls; //地方视午日食

  //日食类型、最大食地标、食分、太阳地平坐标
  if(F.W==100){ //无中心线
   //最大食地标及时分
   ls = this.bse2db([re.xc,re.yc,0],re.I, 0); re.zxJ=ls[0], re.zxW=ls[1]; //最大食地标
   re.sf = (Bc.r2-(re.d-0.9972))/(Bc.r2-Bc.r1); //0.9969是南北极区的平半径
   //类型判断
   if     (re.d>0.9972+Bc.r2)  { re.lx = 'N'; } //无食,半影没有进入
   else if(re.d>0.9972+Bc.ar1) { re.lx = 'P'; } //偏食,本影没有进入
   ...
   re.sf = Bp.sf; //食分
   //类型判断
   if(re.d>0.9966-Bp.ar1) { if(Bp.sf<1) re.lx = 'A1'; else re.lx = 'T1'; } //中心进入,但本影没有完全进入
   else{ //本影全进入有中心日食
    if(Bp.sf>=1){
      re.lx = 'H';
  ...
  re.Sdp = CD2DP(this.sun0,re.zxJ,re.zxW,re.I[2]);  //太阳在中心点的地平坐标

  //食带宽度和时延
  if(F.W!=100){
    re.dw = abs(2*Bp.r1*cs_rEar) / sin(re.Sdp[1]); //食带宽度
    ls = this.Vxy(re.xc,re.yc,re.I[1], re.vx,re.vy); //求地表影速
    re.tt = 2*abs(Bp.r1)/ls.V; //时延
  } else re.dw = re.tt =0;
  return re;
 };

 //界线图
 this.push=function(z,p){ p[p.length]=-z[0], p[p.length]=z[1]; }; //经度改为东经为正,所以有个负号
 this.mQie=function(M,vx0,vy0, h, r,I, A){ //vx0,vy0为影足速度(也是整个影子速度),h=1计算北界,h=-1计算南界
   var x=M[0], y=M[1]+r, c, i;
   for(i=0;i<3;i++){
    c = this.Vxy(x,y,I[1], vx0,vy0);
    x = M[0] - h*r*c.sinA, y = M[1] + h*r*c.cosA;
   }
   var X = M[0] - h*cs_k*c.sinA, Y = M[1] + h*cs_k*c.cosA;
   var p = lineEar2(X,Y,M[2]/cs_rEar,  x,y,0,  cs_ba,1,I);

   if(!A.f2) A.f2=0;   A.f = p.W==100?0:1; //记录有无解
   if(A.f2!=A.f){ //补线头线尾
     var g=lineOvl(x,y,c.Vx,c.Vy,this.eR0);
     if(g.n){
      if(A.f) this.push( this.bse2db([g.B[0],g.B[1],0],I,0), A);//有解补线头
      else    this.push( this.bse2db([g.A[0],g.A[1],0],I,0), A);//无解补线尾
     }
   }
   A.f2 = A.f; //记录上次有无解

   if(p.W!=100) this.push([p.J,p.W],A);
 };
 this.mDian=function(M,vx0,vy0, AB, r,I, A,A2){ //日出日没食甚
   var i, p,a=M, R,c;
   for(i=0;i<2;i++){ //迭代求交点
     c = this.Vxy(a[0],a[1],I[1], vx0,vy0);
     p = lineOvl(M[0],M[1],c.Vy,-c.Vx,this.eR0);
     if(!p.n) break;
     if(AB) a=p.A, R=p.R1;
     else   a=p.B, R=p.R2;
   }
   ...
   if(p.n && R<=r){ //有交点
     if(A.n%2==0) A.n++;
     a=this.bse2db([a[0],a[1],0], I,0); //转为地标
     if(A.n==1) this.push(a,A ); //保存第一食甚线A或B根
     if(A.n==3) this.push(a,A2); //保存第二食甚线A或B根
   }else{
     if(A.n%2==1) A.n++;
   }
 };
 this.jieX=function(jd,jing){ //日出日没的初亏食甚复圆线，南北界线等
  var i,j,k, p, ls;
  var re=this.feature(jd,jing);  //求特征参数
  var vx=re.vx, vy=re.vy;  //影速
  ...
  var t=re.jd-3/24, N=200, dt=2*3/24/N;

  var n1=0, n2=0, n3=0, n4=0; //n1切入时序,n2日出食甚时序

  for(i=0;i<=N;i++,t+=dt){
   var M = this.bseM(t);        //此刻月亮贝塞尔坐标(其x和y正是影足)
   var B = this.rSM(M[2],re.Rs);//本半影等
   var r = B.r2, R = this.eR0;  //半影半径,地球平均半径
   var I = this.bse(t);         //贝塞尔坐标参数

   p=ovlOvl(R,M[0],M[1],r); //求两圆交点
   if(n1%2) {if(!p.n) n1++;} else {if(p.n) n1++;}
   if(p.n) { //有交点
    p.A[2]=p.B[2]=0;  p.A=this.bse2db(p.A,I,0);  p.B=this.bse2db(p.B,I,0); //转为地标
    if(n1==1){ this.push(p.A,re.p1); this.push(p.B,re.p2); }//保存第一亏圆界线
    if(n1==3){ this.push(p.A,re.p3); this.push(p.B,re.p4); }//保存第二亏圆界线
   }

   //日出日没食甚线
   this.mDian(M,vx,vy,0,r,I, re.q1,re.q3);
   this.mDian(M,vx,vy,1,r,I, re.q2,re.q4);

   //求中心线
   p = this.bseXY2db(M[0],M[1],I,1);
   if(p[1]!=100&&n4==0){ //首坐标
     ls=lineOvl(M[0],M[1],vx,vy,R);
     ls.B[2]=0; ls=this.bse2db(ls.B,I,0); //严格的说，这里的t还应提前一些
     this.push(ls,re.L0);
     n4=1;
   }
   if(p[1]==100&&n4==1){ //末坐标
     ls=lineOvl(M[0],M[1],vx,vy,R);
     ls.A[2]=0; ls=this.bse2db(ls.A,I,0); //严格的说，这里的t还应拉后一些
     this.push(ls,re.L0);
     n4=2;
   }
   ...
   this.mQie(M,vx,vy, +1, B.r1,       I, re.L3); //本影北界
   this.mQie(M,vx,vy, -1, B.r1,       I, re.L4); //本影南界
   this.mQie(M,vx,vy, +1, (r+B.r1)/2, I, re.L5); //0.5半影北界
   this.mQie(M,vx,vy, -1, (r+B.r1)/2, I, re.L6); //0.5半影南界
  }
  return re;
 };
 this.jieX2=function (jd,jing){ //jd力学时
  var re=new Object();
  re.p1=new Array(), re.p2=new Array(), re.p3=new Array();

  this.init(jd,jing); //jing=0高精度根数计算
  if(abs(jd-this.Zjd)>0.5) return re;

  var i,s,p,x,y,X,Y;
  var Rs = this.sun0[2];     //太阳平均向径
  var S = this.sun(jd);      //此刻太阳赤道坐标
  var M = this.bseM(jd);     //此刻月亮
  var B = this.rSM(M[2], Rs);//本半影等
  var I = this.bse(jd);      //贝塞尔坐标参数
  var Z = M[2]/cs_rEar;      //月亮的坐标的z量
  ...
  for(i=0;i<=N;i++){
    ...
    if(p.W!=100) this.push( [p.J,p.W], re.p1 );
    else { if(sqrt(x*x+y*y)>a1) this.push( this.bse2db([x,y,0],I,1), re.p1 ); }
    //半影
    x = M[0] + B.r2*cosS, y = M[1] + B.r2*sinS;
    p = lineEar2(X,Y,Z,  x,y,0,  cs_ba,1,I);
    if(p.W!=100) this.push( [p.J,p.W], re.p2 );
    else { if(sqrt(x*x+y*y)>a2) this.push( this.bse2db([x,y,0],I,1), re.p2 ); }
    //晨昏圈
    p = llrConv([s,0,0],pi_2-S[1]);
    p[0] = rad2rrad( I[2] -p[0]-S[0]-pi_2 );
    this.push(p, re.p3);
  }
  return re;
 };
 return this; //为了Flash9兼容，这样在创建实例不用new
}




var GS1=rsGS(1/24,7); //适用于精确计算，如用于rsPL物件的根数计算器
var GS2=rsGS(0.04,2); //适用于快速计算，可快速提取日食特征
var GS3=rsGS(1/24,3); //适用于界线计算，用于绘制界线图
...
var rsPL={ //日食批量快速计算器
 ...
 secXY:function(jd,L,fa,high,re){ //日月xy坐标计算。参数：jd是力学时,站点经纬L,fa,海拔high(千米)
  ...
  var gst= pGST(jd-deltat,deltat) + ZB.dL*Math.cos(hcjj(jd/36525) + ZB.dE); //真恒星时(不考虑非多项式部分)

  var z;
  //=======月亮========
  z=GS1.moon(jd); re.mCJ=z[0]; re.mCW=z[1]; re.mR=z[2]; //月亮视赤经,月球赤纬
  var mShiJ = rad2rrad(gst - L - z[0]); //得到此刻月亮时角
  parallax(z, mShiJ,fa, high); re.mCJ2=z[0], re.mCW2=z[1], re.mR2=z[2]; //修正了视差的赤道坐标

  //=======太阳========
  z=GS1.sun(jd); re.sCJ=z[0]; re.sCW=z[1]; re.sR=z[2]; //太阳视赤经,太阳赤纬
  ...
  if(this.nasa_r) re.mr*=cs_sMoon_nasa/cs_sMoon; //0.99925;
 ...
 zb0:function(jd){
  ...
  this.P.g = pGST(jd-deltat, deltat) + ZB.dL*Math.cos(E+ZB.dE); //真恒星时(不考虑非多项式部分)
  this.P.S=GS1.sun(jd);
  this.P.M=GS1.moon(jd);

  var t2=jd+60/86400;
  this.Q.g = pGST(t2-deltat,deltat) + ZB.dL*Math.cos(E+ZB.dE);
  this.Q.S=GS1.sun(t2);
  this.Q.M=GS1.moon(t2);
 ...
 nbj:function(jd){ //南北界计算
  ...
  if(V[3]!=100&&V[5]!=100){ //粗算本影南北距离
    ...
    this.Vb = toFixed(cs_rEarA*Math.sqrt(x*x+y*y),0)+'千米';
  }
 }
};
